...
<div class="mainContentIn walletHistory">
    <div class="TabbedMenu clearfix">
	  ...
	  <a href="/dashboard#/wallethistory">All</a>
	  <a ng-repeat="wallet in walletList" index="[wallet.currencyId]" heading="{{wallet.name}}" ng-class="wallet.code == 'INR' ? 'active' :'' "  ng-if="wallet.ref_id" href="/dashboard#/{{ wallet.code | lowercase }}wallet"> {{ wallet.name}}</a>
	</div>
    ...
    <form id="profitability_filter_frm" class="col-md-12"  method="post" action="" >
        ...
        <div class="advanceSearch searchBar">
            ...
            <div class="col-md-4 text-right">
                <input type="button" class="btn greenBtn" value="Search" name="search" id="searchInr"/>
            </div>
        </div>
    </form>
    <!--<a ng-click="orderDetails()" href="javascript:void(0)">hello124</a>-->
    <div class="dataTableData table-responsive clearfix">
        <table id="wallethistoryInr" class="table" cellspacing="0" width="100%">
            <thead>
                <tr>
                    <th>Date</th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        oTable = $('#wallethistoryInr').DataTable({
            "bDestroy": true,
            "responsive": true,
            "iDisplayLength": 50,
