...
if(Yii::app()->clientScript->cache!='off' && !Yii::app()->clientScript->debug) {
	/**
	...
	if(Yii::app()->clientScript->cache=='file') {
		if(Yii::app()->clientScript->cachePath != '')
			$min_cachePath = Yii::app()->clientScript->cachePath;
		else
			$min_cachePath = Yii::app() -> runtimePath.'/minify_cache';
	...
	else if(extension_loaded('apc') && Yii::app()->clientScript->cache=='apc')
	{
	    require dirname(__FILE__) . '/lib/Minify/Cache/APC.php';
	    $min_cachePath = new Minify_Cache_APC();
	}
	else if(extension_loaded('memcache') && Yii::app()->clientScript->cache=='memcache')
	{
 ...
 */
$min_serveOptions['maxAge'] = Yii::app()->clientScript->maxAge;
 ...
 */
//$min_serveOptions['minApp']['allowDirs'] = array('css');

/**
 * Set to true to disable the "f" GET parameter for specifying files.
 * Only the "g" parameter will be considered.
 */
/*if(empty($min_serveOptions['minApp']['allowDirs'])) {
